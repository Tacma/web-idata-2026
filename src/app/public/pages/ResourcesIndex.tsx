...
import { getPublished as getPublishedResources } from '../../../services/resourcesService';

const allowMockFallback = import.meta.env.DEV;

export function ResourcesIndex() {
  const { language, getLocalizedValue } = useLanguage();
  const [resources, setResources] = useState<any[]>(allowMockFallback ? mockResources : []);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setResources(data.length > 0 ? data : (allowMockFallback ? mockResources : []));
        }
      } catch (error) {
        console.error('Error loading resources:', error);
        if (!cancelled) {
          setResources(allowMockFallback ? mockResources : []);
        }
      }
    }
