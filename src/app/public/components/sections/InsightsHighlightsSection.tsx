  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const filtered = section.referencedIds?.length
          ? blogPosts.filter((item: any) => section.referencedIds?.includes(item.id))
          : blogPosts;
        const selected = filtered.length > 0 ? filtered : blogPosts;
        setInsights(selected.slice(0, 4));
      } catch (error) {
        console.error('Error loading insights highlights:', error);
        if (!cancelled) {
          const fallback = allowMockFallback
            ? filterVisibleInsights(mockBlogPosts)
                .filter((item) => section.referencedIds?.includes(item.id))
                .slice(0, 4)
            : [];
                ...
                <div className="relative h-full rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                  ...
                  <div className="relative h-full p-8 flex flex-col justify-between">
                    ...
                    <div>
                      ...
                      <span>{new Date(insights[0].publishedDate || insights[0].published_date || Date.now()).toLocaleDateString(language)}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
