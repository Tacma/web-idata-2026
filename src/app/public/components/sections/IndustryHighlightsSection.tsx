...
export function IndustryHighlightsSection({ 
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const industriesData = await getPublishedIndustries();
        if (!cancelled) {
          const filtered = section.referencedIds && section.referencedIds.length > 0
            ? industriesData.filter((industry: any) => section.referencedIds?.includes(industry.id))
            : industriesData;
          setIndustries(filtered.slice(0, 4));
        }
      } catch (error) {
        console.error('Error loading industry highlights:', error);
        if (!cancelled) {
          setIndustries([]);
        }
      }
              ...
              >
                <div className="flex flex-col h-full min-h-[280px] justify-between">
                  ...
                  <div>
                    ...
                    {/* CTA */}
                    <Link
                      to={`/${language}/${language === 'es' ? 'casos-de-exito' : 'case-studies'}#${getLocalizedValue(industry.slug_es, industry.slug_en)}`}
                      className="inline-flex items-center gap-2 text-gray-900 font-medium text-sm hover:gap-3 transition-all duration-300"
                    >
                      <span>{language === 'es' ? 'Ver casos relacionados' : 'View related cases'}</span>
