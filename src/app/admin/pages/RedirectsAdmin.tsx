    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={creating || !form.from.trim() || !form.to.trim()}
          className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50"
        >
          <Plus className="w-4 h-4" />
          {creating ? 'Creando...' : 'Crear redirección'}
        </button>
      </div>

      <div className="grid gap-3 rounded-xl border border-gray-200 bg-white p-5 md:grid-cols-[1fr,1fr,180px]">
        <input
          value={form.from}
